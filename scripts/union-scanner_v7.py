...
# ================= 📝 结构化日志系统 =================
# 创建自定义 JSON formatter
class StructuredFormatter(jsonlogger.JsonFormatter):
    """自定义结构化日志格式器，包含请求追踪 ID"""
    ...
    async def _process_queue(self):
        ...
        while self._running:
            try:
                ...
                # 等待 interval_seconds 后再处理下一批
                await asyncio.sleep(self.interval_seconds)

            except Exception as e:
                logger.error(f"邮件队列处理异常", extra={'error': str(e)})
                await asyncio.sleep(self.interval_seconds)
...
async def send_email(subject: str, content: str):
    ...
    if EMAIL_QUEUE_ENABLED:
        ...
        try:
            ...
            logger.error(f"邮件发送失败", extra={'subject': subject, 'error': str(e)})
    ...
    def _try_adjust_concurrency(self):
        ...
        if success_rate < (1 - self.error_threshold) and self.current_concurrency > self.min_concurrency:
            ...
            logger.warning(
                ...
                extra={
                    'event': 'adaptive_downgrade',
                    'success_rate': success_rate,
                    'old_concurrency': self.current_concurrency + 1,
...
async def db_add_watchlist(uid, ex_id, symbol, high, low, expiry):
    ...
    async with db_pool.get_connection() as db:
        ...
        await db.commit()
        logger.debug(f"添加监控目标", extra={'uid': uid, 'exchange': ex_id, 'symbol': symbol})
...
async def db_remove_watchlist(uid):
    ...
    async with db_pool.get_connection() as db:
        ...
        await db.commit()
        logger.debug(f"删除监控目标", extra={'uid': uid})
...
async def db_update_alert(uid, alert_time):
    ...
    async with db_pool.get_connection() as db:
        ...
        await db.commit()
        logger.debug(f"更新提醒历史", extra={'uid': uid})
    ...
    for ex_id, params in EXCHANGES_TO_LOAD.items():
        if hasattr(ccxt, ex_id):
            try:
                ...
                logger.warning(f"初始化失败", extra={'exchange': ex_id, 'error': str(e)})
...
async def get_global_targets():
    ...
    async def fetch_markets(ex_id, ex):
        ...
        try:
            ...
            logger.warning(f"无法拉取市场数据", extra={'exchange': ex_id, 'error': str(e)})
...
async def check_structure(ex, symbol):
    ...
    async with adaptive_controller.semaphore:  # 使用自适应并发控制器
        ...
        try:
            ...
            adaptive_controller.record_request(False)  # 记录失败
            logger.debug(f"形态检测失败", extra={'exchange': ex.id, 'symbol': symbol, 'error': str(e)})
            return None
...
async def radar_job():
    ...
    while True:
        try:
            ...
            logger.error(f"雷达任务异常", extra={'error': str(e)})
...
async def sniper_job():
    ...
    while True:
        ...
        if watchlist:
            # logger.info(f"狙击手巡视中", extra={'target_count': len(watchlist)})
            tasks = []
...
async def snip_single_target(uid, ex, symbol, info):
    ...
    async with adaptive_controller.semaphore:
        ...
        try:
            ...
            adaptive_controller.record_request(False)  # 记录失败
            logger.debug(f"价格查询失败", extra={'exchange': ex.id, 'symbol': symbol, 'error': str(e)})
            pass # 网络抖动，忽略，等下一个 60 秒

async def main():
    print("===================================================")
    print("🚀 万物并集猎手 V17.0 (邮件队列版) 启动...")
    print("===================================================")
