...
SENDER_PASSWORD = 'gjxqxydprjjpbija'
...
def load_data_from_db():
    ...
    try:
        ...
        logging.info(f"🔄 数据库恢复: {len(watchlist)} 个盯盘任务, {len(pending_confirms)} 个待确认任务.")
    except Exception as e:
...
def sync_send_email(subject, content, is_html=False):
    try:
        ...
        msg['From'] = formataddr(["V24战术猎手(测试)", SENDER_EMAIL])
    ...
    async def fetch_markets(ex_id, ex):
        try:
            ...
            try:
                ...
                logging.error(f"{ex_id} 获取tickers失败: {e}")
                return
    ...
    while True:
        try:
            ...
            if targets:
                ...
                if count_new > 0:
                    logging.info(f"🤫 雷达静默锁定 {count_new} 个新目标 (已入库，等待狙击)")
...
async def snip_target(uid, info):
    try:
        ...
        if signal:
            logging.info(f"⚡ {symbol} 瞬时突破 ({price})，正在二审...")

            # --- V22 二审逻辑 (前序收敛检查) ---
            recent_bars = await ex.fetch_ohlcv(symbol, '1h', limit=12)
            ...
            if check_vol_avg > base_vol * 1.2:
                logging.warning(f"❌ {symbol} 二审失败 (基座不稳)，移除盯盘。")
                del watchlist[uid]
                db_crud('remove_watch', {'uid': uid})
                return

            # --- 二审通过，进入报警流程 ---
            logging.info(f"✅ {symbol} 二审通过！发送瞬时报警。")
            ...
            <p>⚠️ <b>注意：</b>此为瞬时信号。系统将在整点收线后确认是否有效。</p>
            """
            asyncio.create_task(send_email(f"🚀 [测试] {signal} {symbol}", content, is_html=True))

            current_ts = int(time.time() * 1000)
            candle_ts = current_ts - (current_ts % 3600000)

            pending_data = {
                'uid': uid, 'ex_id': ex.id, 'symbol': symbol,
            ...
            if pending_confirms:
                ...
                for uid in list(pending_confirms.keys()):
                    ...
                    if current_ts > (item['candle_ts'] + 3600000 + 60000):
                        ...
                        candle_data = await ex.fetch_ohlcv(symbol, '1h', since=item['candle_ts'], limit=1)
                ...
                if confirmed_list:
                    ...
                    await send_email(f"✅ [测试] {len(confirmed_list)} 个突破已确认有效", html, is_html=True)

        except Exception as e:
            logging.error(f"确认任务异常: {e}")
        await asyncio.sleep(60)

async def main():
    print("🚀 V24.0 战术猎手启动 (测试模式)")
    print("模式: 雷达静默 | 瞬时报警 | 收线确认")
    init_db()
